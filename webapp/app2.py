...
def load_models():
    global yolo_model, card_model, a4paper_model, pose_model, sam_model, device
    device = get_device()
    ...
    # Load pose model for accurate ankle/heel keypoint detection
...
def set_mode():
    global detection_mode, card_bbox, distance, focal_length, marker_corners, aruco_pixel_side
    data = request.json
...
def handle_start_stream():
    global cap, streaming, frame_count, person_bbox, is_cut_off, warning_message, distance, focal_length, marker_corners, all_persons, selected_person_idx, card_bbox, a4paper_bbox, a4paper_pixel_length, a4paper_obb_corners, aruco_pixel_side
...
def stream_loop():
    global cap, streaming, frame_count, person_bbox, is_cut_off, distance, focal_length, marker_corners, warning_message, all_persons, selected_person_idx, card_bbox, a4paper_bbox, a4paper_pixel_length, a4paper_obb_corners, aruco_pixel_side
            ...
            if detection_mode == "aruco":
                ...
                if marker_IDs is not None:
                    for ids, corners in zip(marker_IDs, detected_corners):
                        ...
                        tvec = tvec.flatten()
                        distance = float(math.sqrt(tvec[0]**2 + tvec[1]**2 + tvec[2]**2))
                        focal_length = fl

                        # Also calculate pixel side length for more accurate ratio-based measurement
                        c_pts = corners.reshape(4, 2)
                        s1 = np.linalg.norm(c_pts[0] - c_pts[1])
                        s2 = np.linalg.norm(c_pts[1] - c_pts[2])
                        s3 = np.linalg.norm(c_pts[2] - c_pts[3])
                        s4 = np.linalg.norm(c_pts[3] - c_pts[0])
                        aruco_pixel_side = float((s1 + s2 + s3 + s4) / 4.0)
                else:
                    distance = None
                    focal_length = None
                    aruco_pixel_side = None
...
@socketio.on("capture")
def handle_capture():
    global cap, streaming, person_bbox, is_cut_off, distance, focal_length, all_persons, card_bbox, a4paper_bbox, a4paper_pixel_length, aruco_pixel_side
...
def get_pose_bounds(frame, bbox, mask_resized, polygon_fallback):
    """Use YOLO pose to get accurate head crown (no hair) and foot bottom (heel).

    HEAD TOP: Estimates the crown of the skull using nose (kp 0) and eye (kp 1,2)
    keypoints.  Crown ≈ eye_y − 1.6 × (nose_y − eye_y).  This sits below the
    hair, giving a hair-free height measurement.

    FOOT BOTTOM: Takes the lower ankle keypoint (kp 15/16), then extends
    DOWN through the SAM mask in a narrow strip around the ankle X to reach
    the actual heel contact point (the ankle joint itself is above the heel).

    Args:
        frame:         Full BGR frame.
        bbox:          [x1, y1, x2, y2] YOLO person bounding box.
        mask_resized:  Binary 2-D numpy array (H × W) from SAM at frame size.
        polygon_fallback: SAM contour Nx2, used when pose detection fails.
    Returns:
        (head_top_y, foot_bottom_y): Y pixel coordinates.
        head_top_y may be None if pose head keypoints are unavailable.
    """
    fallback_bottom = int(np.max(polygon_fallback[:, 1]))
    try:
        pose_results = pose_model(frame, device=device, verbose=False, imgsz=640)
    except Exception:
        return None, fallback_bottom

    kpts_obj = pose_results[0].keypoints
    boxes_obj = pose_results[0].boxes
    if kpts_obj is None or len(kpts_obj) == 0:
        return None, fallback_bottom

    # Match detected pose to target person by bounding-box centre distance
    x1, y1, x2, y2 = bbox
    bbox_cx, bbox_cy = (x1 + x2) / 2, (y1 + y2) / 2
    best_idx, min_dist = 0, float('inf')
    for i, box in enumerate(boxes_obj):
        bx1, by1, bx2, by2 = box.xyxy[0].tolist()
        dist = math.hypot((bx1 + bx2) / 2 - bbox_cx, (by1 + by2) / 2 - bbox_cy)
        if dist < min_dist:
            min_dist = dist
            best_idx = i

    kp_xy  = kpts_obj.xy[best_idx].cpu().numpy()   # (17, 2)
    kp_conf = kpts_obj.conf[best_idx].cpu().numpy() # (17,)
    h_mask, w_mask = mask_resized.shape

    # --- HEAD CROWN (exclude hair) ---
    # Use nose (0) + eyes (1, 2): crown ≈ eye_y - 1.6 × (nose_y - eye_y)
    head_top_y = None
    nose_y = float(kp_xy[0][1]) if kp_conf[0] > 0.3 and kp_xy[0][1] > 0 else None
    eye_ys = [float(kp_xy[i][1]) for i in [1, 2]
               if kp_conf[i] > 0.3 and kp_xy[i][1] > 0]
    if eye_ys and nose_y is not None:
        avg_eye_y = float(np.mean(eye_ys))
        eye_to_nose = nose_y - avg_eye_y      # positive: nose is below eyes
        if eye_to_nose > 5:
            crown_y = avg_eye_y - 2.5 * eye_to_nose
            head_top_y = int(max(0, crown_y))
            print(f"  Crown: {head_top_y}  (eye={avg_eye_y:.0f}, nose={nose_y:.0f})")

    # --- FOOT BOTTOM (actual heel, ankle joint + mask below) ---
    foot_bottom_y = fallback_bottom
    # Find the lower ankle (larger Y = closer to ground)
    best_ankle = None
    for idx in [15, 16]:  # left_ankle, right_ankle
        if kp_conf[idx] > 0.3 and kp_xy[idx][0] > 0 and kp_xy[idx][1] > 0:
            if best_ankle is None or kp_xy[idx][1] > best_ankle[1]:
                best_ankle = kp_xy[idx]

    if best_ankle is not None:
        ay = int(best_ankle[1])
        toe_tip_y = fallback_bottom  # absolute bottom of SAM mask = toe tip
        # Midpoint between ankle joint and toe tip ≈ heel contact point
        foot_bottom_y = (ay + toe_tip_y*2) // 3
        print(f"  Ankle kp y={ay}, toe_tip y={toe_tip_y}, foot_bottom y={foot_bottom_y}")

    return head_top_y, foot_bottom_y


def _capture_aruco():
    """Capture and measure height using ArUco marker as reference (single person)."""
    if aruco_pixel_side is None:
        emit("capture_result", {"success": False, "error": "No ArUco marker detected for calibration"})
        return
    ...
    if sam_results[0].masks is not None and len(sam_results[0].masks.xy) > 0:
        ...
        # Compute binary mask first — needed for ankle/heel detection
        mask = sam_results[0].masks.data[0].cpu().numpy()
        mask_resized = cv2.resize(mask.astype(np.uint8), (w, h))
        # Use pose for both head crown (no hair) and actual heel (ankle + mask)
        yolo_top = person_bbox[1]
        yolo_bottom = person_bbox[3]
        head_top_y, foot_bottom_y = get_pose_bounds(frame, person_bbox, mask_resized, polygon)
        if head_top_y is not None:
            highest_y = max(head_top_y, yolo_top)  # crown sits below hair, above YOLO top
        else:
            highest_y = max(int(np.min(polygon[:, 1])), yolo_top)  # fallback
        lowest_y = min(foot_bottom_y, yolo_bottom)

        pixel_height = lowest_y - highest_y
        estimated_height = (pixel_height * MARKER_SIZE) / aruco_pixel_side
        print("head_top_y", head_top_y, "foot_bottom_y", foot_bottom_y, "pixel_height", pixel_height)
        ...
        emit("capture_result", {
            ...
            "mode": "aruco",
            "image": frame_data,
            "height_cm": round(float(estimated_height), 1),
...
def _capture_card():
    ...
    for i, bbox in enumerate(persons_to_process):
        ...
        yolo_top = bbox[1]
        yolo_bottom = bbox[3]
        head_top_y, foot_bottom_y = get_pose_bounds(frame, bbox, mask_resized, polygon)
        if head_top_y is not None:
            highest_y = max(head_top_y, yolo_top)
        else:
            highest_y = max(int(np.min(polygon[:, 1])), yolo_top)
        lowest_y = min(foot_bottom_y, yolo_bottom)
